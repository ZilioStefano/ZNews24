...
def createPowerGauge(Data):
    ...
    lastP = float(Data[0])
    PMax = float(Data[1])
    PMinus = float(Data[2]-Data[3])
    PPlus = float(Data[2]+Data[3])
    ...
    fig = pn.indicators.Gauge(
        name="P", value=shownP, bounds=(0, PMax), format='{value} kW',
...
def createEtaGauge(Data):
    ...
    lastEta = 100 * Data[0]
    etaMax = Data[1]
    etaMinus = 100 * (Data[2] - Data[3])
    etaPlus = 100 * (Data[2] + Data[3])
    ...
    if np.isnan(lastEta) == 0:
        shownEta = round(lastEta, 1)
    else:
        shownEta = 0

    fig = pn.indicators.Gauge(
        name=name, value=shownEta, bounds=(0, etaMax), format='{value} %',
        colors=[(etaMinus/etaMax, 'red'), (etaPlus/etaMax, 'gold'), (1, 'green')], annulus_width=5,
        custom_opts={"pointer": {"itemStyle": {"color": "black"}, 'icon': pointer_path}, "value": {"color": "black"},
